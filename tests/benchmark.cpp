...
flatbuffers::unique_ptr_t CreateFlatBufferTest(std::string &buffer) {
    ...
    auto vecofstrings =
            builder.CreateVector<flatbuffers::Offset<flatbuffers::String>>(4,
                                                                           [&](size_t i) {
                                                                               return builder.CreateSharedString(
                                                                                       names[i]);
                                                                           });
...
std::pair<uint64_t, uint8_t *>  BuildFlatbuffer() {
    ...
    auto scores1_subject = builder.CreateString("Math");
    float scores1_score = 7;
    auto score1 = CreateScore(builder, scores1_subject, scores1_score);
    auto scores2_subject = builder.CreateString("Physics");
    float scores2_score = 8;
    auto score2 = CreateScore(builder, scores2_subject, scores2_score);
    std::vector<flatbuffers::Offset<Score>> scores;
    scores.push_back(score1);
    scores.push_back(score2);
    auto student4 = builder.CreateVector(scores);

    auto avg_subject = builder.CreateString("Average score");
    float avg_score = 7.5;
    auto student5 = CreateScore(builder, avg_subject, avg_score);

    int8_t school_day[] = {2, 3, 5, 7};

//    auto student6 = builder.(school_day);
    auto student6 = builder.CreateVector<int8_t>(school_day, sizeof(school_day));
    auto offset = CreateStudent(builder, student1, student2, student3, student4, student5, student6);
    builder.Finish(offset);
    return std::make_pair(builder.GetSize(), builder.GetBufferPointer());
}
...
//
//int main() {
//    const std::vector<uint8_t> buf = BuildSchemaless().second;
//    auto data = schemaless::GetRoot(buf).AsMap();
//
//    printf("%s", data["name"].AsString().c_str());
//
//
//
...
////    std::cout << "Build Regular FlatBuffers size = " << BuildFlatbuffer().first << std::endl;
////
////
...
////    std::cout << "Build Schemaless FlatBuffers size = " << BuildSchemaless().first << std::endl;
//    return 0;
//}
